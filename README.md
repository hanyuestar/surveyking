...
- **安全加固**
  - 自助修改密码现在必须提供正确的旧密码，且只能修改自己的账户，修复了可越权重置他人密码的隐患。
  - 改密后旧的登录态会立即失效，避免改密前的令牌被继续滥用。
  - 管理员创建用户和批量导入用户时强制要求强密码。
  - 外挂密码重置接口增加了按 IP 的限流，连续失败会临时拦截，防止弱口令被暴力破解。
  - 检测到仍在使用默认弱口令（`GOD_SECRET=super666`）启动时会打印告警，提醒生产环境换成强口令。
- **内置问卷模板**：新增 8 个开箱即用的整卷模板，覆盖满意度 / 反馈问卷、报名 / 信息登记、在线考试（自动判分）、投票 / 评选各 2 个。首次启动自动写入系统模板库，重复启动不会重复灌入。模板均为原创内容。
